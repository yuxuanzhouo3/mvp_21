  ...
  const handleStartPayment = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || (isEn ? 'Failed to create payment' : '创建支付失败'));
      }

      const data = await response.json();

      if (!data.success || !data.code_url) {
        throw new Error(isEn ? 'Invalid response from server' : '服务端返回无效');
      }
      ...
      const errorMessage = error instanceof Error ? error.message : isEn ? 'Unknown error' : '未知错误';
    ...
    const poll = async () => {
      try {
        ...
        if (Date.now() - startTime > timeout) {
          throw new Error(isEn ? 'Payment status check timeout. Please try again.' : '支付状态查询超时，请重试。');
        }
  ...
  return (
    <div className="flex flex-col items-center gap-6 w-full max-w-md mx-auto p-6">
      ...
      {state.status === 'idle' && (
        ...
        >
          {isEn ? `Pay with WeChat ¥${amount.toFixed(2)}` : `微信支付 ¥${amount.toFixed(2)}`}
        </button>
      )}
      ...
      {state.status === 'loading' && (
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-gray-600">{isEn ? 'Generating QR code...' : '生成二维码中...'}</p>
        </div>
      )}
      ...
      {(state.status === 'showing_qr' || state.status === 'polling') && state.codeUrl && (
        <div className="flex flex-col items-center gap-4 w-full">
          <div className="border-2 border-gray-300 p-4 rounded-lg bg-white">
            <QRCodeSVG value={state.codeUrl} size={256} level="H" includeMargin={true} />
          </div>

          <div className="text-center">
            <h3 className="text-lg font-semibold mb-2">{isEn ? 'Scan to complete payment' : '扫一扫完成支付'}</h3>
            <p className="text-sm text-gray-600 mb-3">{isEn ? 'Use WeChat to scan the QR code above' : '用微信扫描上面的二维码'}</p>

            {remainingTime !== null && (
              <p className="text-sm text-orange-600">
                {isEn ? 'QR expires in' : '二维码有效期'}: {formatTime(remainingTime)}
              </p>
            )}

            {state.status === 'polling' && state.pollCount && (
              <p className="text-xs text-gray-400 mt-2">
                {isEn ? `Checked ${state.pollCount} times...` : `已检查 ${state.pollCount} 次...`}
              </p>
            )}
          </div>
      ...
      {state.status === 'success' && (
        <div className="flex flex-col items-center gap-4 w-full py-8">
          ...
          <h3 className="text-2xl font-bold text-green-600">{isEn ? 'Payment Successful' : '支付成功'}</h3>
          <p className="text-gray-600 text-center">
            {isEn ? 'Thanks for your purchase. Your account has been upgraded to Pro.' : '感谢您的购买，您已升级为 Pro 用户'}
          </p>
          <p className="text-sm text-gray-500 break-all">
            {isEn ? 'Order ID' : '订单号'}: {state.outTradeNo}
          </p>
      ...
      {state.status === 'error' && (
        <div className="flex flex-col items-center gap-4 w-full py-8">
          <div className="text-5xl">❌</div>
          <h3 className="text-2xl font-bold text-red-600">{isEn ? 'Payment Failed' : '支付失败'}</h3>
