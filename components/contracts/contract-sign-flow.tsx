...
export function ContractSignFlow({
  ...
  const resolvedBackLabel =
    backLabel || (isEn ? "Back to Contracts" : "返回合同列表");
  ...
  function buildSignatureInput(role: "sender" | "counterparty") {
    ...
    if (signMethod === "type") {
      ...
      return { id: buildId("signature"), role, method: "type", signerName, legalConsent, typedName: typedName.trim(), createdAt, source: signatureSource };
    }
    ...
    return { id: buildId("signature"), role, method: "draw", signerName, legalConsent, imageDataUrl, imageMimeType: "image/png", createdAt, source: signatureSource };
  }
  ...
  async function handlePrimaryAction() {
    ...
    if (enhancement.signFlow.status === "draft") {
      try {
        setSubmitting(true);
        await applyContractActionForCurrentUser(contractId, "start_signing", isEn ? "Signing launched from shared sign flow." : "已从统一签署页发起签署。");
      } catch (actionError) {
      ...
      setSubmitting(false);
      await confirm("sender", "confirm_sender", isEn ? "Sender confirmation recorded from shared sign flow." : "已从统一签署页记录发起方签署。");
      return;
    }
    ...
    try {
      ...
      if (enhancement.signFlow.status === "awaiting_counterparty") {
        const updated = await applyContractActionForCurrentUser(contractId, "send_reminder", isEn ? "Reminder sent from shared sign flow." : "已从统一签署页发送提醒。");
        setContract(updated);
  ...
  async function handleCounterpartyConfirmed() {
    await confirm("counterparty", "confirm_counterparty", isEn ? "Counterparty confirmation recorded from shared sign flow." : "已从统一签署页记录对方签署。");
  }
            ...
            <Card>
              <CardContent className="space-y-4 pb-4 pt-4">
                ...
                <div className="flex items-start gap-3 rounded-lg border border-border/70 bg-muted/15 p-3">
                  <Checkbox id="sign-consent" checked={legalConsent} onCheckedChange={(checked) => setLegalConsent(checked === true)} />
                  <div className="space-y-1">
                    <Label htmlFor="sign-consent" className="text-sm leading-6">{isEn ? "I confirm this electronic signature has the same legal effect as my handwritten signature." : "我确认该电子签名与本人手写签名具有同等法律效力。"}</Label>
                    <p className="text-xs text-muted-foreground">{isEn ? "The selected signature method, timestamp, and retained evidence will be written back to the contract." : "当前签署方式、时间戳和留痕证据将写回合同记录。"}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardContent className="space-y-2 pb-4 pt-4">
              ...
              <p className="pt-1 text-center text-xs text-muted-foreground">{isEn ? "Signing records are synced with the main contract workflow." : "签署记录会与主合同流程同步。"}</p>
            </CardContent>
          </Card>
        </div>
      </div>
