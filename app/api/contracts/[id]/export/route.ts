...
export async function GET(request: NextRequest, context: RouteContext) {
  ...
  try {
    ...
    if (auth.error) {
      observe("rejected", auth.error.status || 401, { reason: "auth_required", format });
      return auth.error;
    }
    ...
    if (!contract) {
      observe("rejected", 404, { reason: "contract_not_found", contractId: id, format }, auth.user.id);
    ...
    if (!assertContractAccess(contract.userId, auth.user)) {
      observe("rejected", 403, { reason: "forbidden", contractId: id, format }, auth.user.id);
      return NextResponse.json(
    ...
    if (!content) {
      observe("rejected", 400, { reason: "empty_content", contractId: id, format }, auth.user.id);
      return NextResponse.json(
    ...
    observe("success", 200, { format, contractId: id }, auth.user.id);
