...
function EditPageContent() {
  ...
  useEffect(() => {
    ...
    async function loadDraft() {
      ...
      try {
        ...
        if (!content) {
          toast.error(isEn ? "Please generate the contract first." : "请先生成合同内容");
          router.replace(analyzeHref);
          return;
        }

        const renderedHtml =
          typeof record.metadata?.editorHtml === "string" ? record.metadata.editorHtml : null;
        ...
        if (!cancelled) {
          ...
          toast.error(
            error instanceof Error ? error.message : isEn ? "Failed to load contract" : "加载合同失败",
          );
          router.replace(fallbackHref);
        }
      } finally {
  ...
  const persistDraft = async (silent = false) => {
    ...
    const previousHtml =
      typeof contractRecord.metadata?.editorHtml === "string" ? contractRecord.metadata.editorHtml : "";
    ...
    const nextMetadata = hasMeaningfulChange
      ? appendContractVersionHistory(
          ...
          createVersionEntry({
            action: "draft_saved",
            title: contractTitle,
            summary: isEn ? "Saved edits in the contract editor." : "已保存本次编辑内容。",
          }),
        )
      : { ...(contractRecord.metadata || {}) };
  ...
  const contractTypeLabel =
    CONTRACT_TYPE_NAMES[contract.contractType || "custom"] ||
          ...
          <CardHeader className="pb-4">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-lg">{isEn ? "Contract Draft Ready" : "合同草稿已生成"}</CardTitle>
            ...
            <CardContent className="p-0">
              <TabsContent value="edit" className="m-0">
                <div
                  ...
                  onInput={(event) => setEditedContent((event.target as HTMLDivElement).innerHTML)}
                />
              </TabsContent>
