-- ==========================================
-- ContractHub 国际版 Supabase 数据库设置（简化版）
-- 先创建所有表，再设置安全策略
-- ==========================================
...
CREATE POLICY IF NOT EXISTS "Users can create own contracts" ON contracts
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update own contracts" ON contracts
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete own contracts" ON contracts
  FOR DELETE USING (auth.uid() = user_id);

-- 合同模板策略
CREATE POLICY IF NOT EXISTS "Public templates are viewable by everyone" ON contract_templates
  FOR SELECT USING (is_public = true OR auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can create own templates" ON contract_templates
  FOR INSERT WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update own templates" ON contract_templates
  FOR UPDATE USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can delete own templates" ON contract_templates
  FOR DELETE USING (auth.uid() = user_id);
