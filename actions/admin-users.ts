...
function isFreePlan(plan: unknown): boolean {
...
function isProPlan(plan: unknown): boolean {
  const normalized = normalizePlan(plan);
  return normalized === "pro" || normalized === "monthly" || normalized === "yearly";
}

function isEnterprisePlan(plan: unknown): boolean {
...
export async function getUserStats(): Promise<ApiResponse<{
  ...
  try {
    ...
    const free = allUsers.filter(u =>
      isFreePlan(u.subscription_plan)
    ).length;

    const pro = allUsers.filter(u =>
      isProPlan(u.subscription_plan)
    ).length;

    const enterprise = allUsers.filter(u =>
      isEnterprisePlan(u.subscription_plan)
    ).length;
    ...
    const paidUsersByRegion = {
      domestic: allUsers.filter((u) =>
        ...
        (isProPlan(u.subscription_plan) || isEnterprisePlan(u.subscription_plan))
      ).length,
