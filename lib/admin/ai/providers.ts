...
async function callOpenAiJson(input: TextGenerationInput): Promise<any> {
  const key = getRequiredEnv('OPENAI_API_KEY')
  const route = resolveAiProviderRoute(input.region)
  const model = input.model || route.analysisModel
  const json = await fetchJson(`${getOpenAiApiBase()}/chat/completions`, {
    method: 'POST',
    headers: {
...
export async function generateStructuredJson(input: TextGenerationInput): Promise<any> {
  ...
  if (route.analysisProvider === 'aliyun-bailian') {
    return callQwenJson(input)
  }

  if (route.analysisProvider === 'openai') {
    return callOpenAiJson(input)
  }

  if (route.analysisProvider === 'gemini') {
    return callGeminiJson(input)
  }

  throw new Error(`Unsupported analysis provider: ${route.analysisProvider}`)
}
