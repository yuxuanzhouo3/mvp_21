...
export class SupabaseAdminAdapter implements AdminDatabaseAdapter {
  ...
  async getAdStats(): Promise<AdStats> {
    ...
    const adsData = (data ?? []) as Array<{ status?: string; type?: string }>;

    const stats: AdStats = {
      total: adsData.length,
      active: adsData.filter(ad => ad.status === 'active').length,
      inactive: adsData.filter(ad => ad.status === 'inactive').length,
      byType: {
        image: adsData.filter(ad => ad.type === 'image').length,
        video: adsData.filter(ad => ad.type === 'video').length,
      },
    };
